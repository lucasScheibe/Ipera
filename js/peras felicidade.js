document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const botoesQuantidade = document.querySelectorAll(".quantidade1");
  
    botoesQuantidade.forEach(container => {
      const btnMenos = container.querySelectorAll(".btn-quantidade")[0];
      const btnMais = container.querySelectorAll(".btn-quantidade")[1];
      const spanQuantidade = container.querySelector(".quantidade");
  
      btnMenos.addEventListener("click", () => {
        let qtd = parseInt(spanQuantidade.textContent);
        if (qtd > 1) {
          spanQuantidade.textContent = qtd - 1;
        }
      });
